...
//       } else {
//         newSelected[name] = { grams, quantity: 1 };
//       }
//       const newTotal = Object.values(newSelected).reduce(
//         (acc, sweet) => acc + sweet.grams * sweet.quantity,
//         0
//       );
...
//     setSelectedSweets((prev) => {
//       const newSelected = { ...prev, [name]: { ...prev[name], grams } };
//       const newTotal = Object.values(newSelected).reduce(
//         (acc, sweet) => acc + (sweet.grams || 0) * sweet.quantity,
//         0
//       );
//       setTotalGrams(newTotal);
//       return newSelected;
...
//     setSelectedSweets((prev) => {
//       const newSelected = { ...prev, [name]: { ...prev[name], quantity } };
//       const newTotal = Object.values(newSelected).reduce(
//         (acc, sweet) => acc + sweet.grams * sweet.quantity,
//         0
//       );
...
//                   <TextField
//                     label={sweet.name}
//                     value={selectedSweets[sweet.name]?.grams || sweet.grams}
//                     onChange={(e) =>
...
//                   <TextField
//                     label="Quantity"
//                     value={selectedSweets[sweet.name]?.quantity || 1}
//                     onChange={(e) =>
//                       handleQuantityChange(sweet.name, parseInt(e.target.value))
//                     }
//                     type="number"
...
const SweetsForm = ({ formData, onBack, onSubmit }) => {
  ...
  // Set max grams based on formData.weight
  useEffect(() => {
    const weightMap = { "1kg": 1000, "500gm": 500, "250gm": 250 };
    setMaxGrams(weightMap[formData.weight] || 0);
  }, [formData.weight]);
              ...
              label={
                <div>
                  <TextField
                    ...
                    fullWidth
                  />
                  {/* Add space between grams and Quantity */}
                  <br />
                  <TextField
                    label="Quantity"
                    value={selectedSweets[sweet.name]?.quantity || 1}
